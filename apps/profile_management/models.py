...
class Notification(models.Model):
    ...
    # Fields
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    user_id = models.CharField(max_length=255, null=False, blank=False)  # Assuming user_id is a string reference
    ...
    related_id = models.CharField(max_length=255, null=True, blank=True)  # Nullable field
...
class Internship(models.Model):
    ...
    # Fields
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    business_id = models.CharField(max_length=255, null=True, blank=True)  # Assuming string reference to Business
    ...
    skill_level_required = models.IntegerField(null=True, blank=True)  # 1-5, nullable
    status = models.CharField(max_length=20, choices=STATUS_CHOICES, default='DRAFT')
    stipend = models.FloatField(null=True, blank=True)  # Optional, using FloatField for Double
    hours_per_week = models.IntegerField(null=True, blank=True)
    application_process = models.TextField(null=True, blank=True)
    applicants = models.ManyToManyField('students.Student', related_name='internships', blank=True)
...
class CareerPath(models.Model):
    ...
    recommended_courses = ArrayField(models.CharField(max_length=255), default=list)
    potential_jobs = ArrayField(models.CharField(max_length=255), default=list)
    average_salary = models.FloatField(null=True, blank=True)  # Double in Java -> Float in Django
    ...
    matching_internships = ArrayField(models.CharField(max_length=255), default=list)  # Internship IDs as strings
...
class AssessmentResult(models.Model):
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    student_id = models.CharField(max_length=100, help_text="Reference to Student")
    assessment_id = models.CharField(max_length=100, help_text="Reference to AssessmentGame")
    ...
    badges_earned = models.JSONField(default=list, help_text="List of Badge IDs")  # Using JSONField for simplicity
...
class AssessmentGame(models.Model):
    ...
    title = models.CharField(max_length=255)
    description = models.TextField(blank=True, null=True)
    skill_category = models.CharField(max_length=100, db_column="skill_category")
    difficulty = models.IntegerField(choices=[(i, str(i)) for i in range(1, 6)], help_text="Difficulty level from 1 to 5")
    time_limit = models.IntegerField(db_column="time_limit", help_text="Time limit in minutes")
    points_available = models.IntegerField(db_column="points_available")
    completion_badge_id = models.CharField(max_length=100, db_column="completion_badge_id", blank=True, null=True, help_text="Reference to Badge")

class Question(models.Model):
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    assessment_game = models.ForeignKey(AssessmentGame, on_delete=models.CASCADE, related_name="questions")
    text = models.TextField()  # Assuming a basic question structure
    correct_answer = models.CharField(max_length=255, blank=True, null=True)  # Adjust based on your needs
...
class ARRoom(models.Model):
    ...
    available_to_levels = models.JSONField(default=list)  # Stores list of integers
...
class ObjectModel(models.Model):
    ...
    ar_room = models.ForeignKey(ARRoom, on_delete=models.CASCADE, related_name='object_models')
...
class InteractionPoint(models.Model):
    id = models.BigAutoField(primary_key=True)
    ar_room = models.ForeignKey(ARRoom, on_delete=models.CASCADE, related_name='interaction_points')
...
class AIInteraction(models.Model):
    ...
    student_id = models.CharField(max_length=255, blank=True, null=True)  # Adjust max_length as needed
...
class Student(User):
    user_id = models.CharField(max_length=36, unique=True)  # Matches Java's String userId
    skill_level = models.IntegerField(null=True, blank=True, choices=[(i, str(i)) for i in range(1, 6)])  # 1-5
    total_points = models.IntegerField(default=0)
    interests = models.TextField(blank=True, help_text="Comma-separated list of interests")  # Alternative to ElementCollection
    education_level = models.CharField(max_length=100, blank=True, null=True)
    guardian_id = models.CharField(max_length=36, blank=True, null=True)  # Reference to another User
    mentor_id = models.CharField(max_length=36, blank=True, null=True)  # Reference to another User

class Guardian(models.Model):
    # Primary key referencing a User (assumed as a string ID)
    user_id = models.CharField(max_length=255, primary_key=True)  # Adjust length as needed

    # List of student IDs stored as JSON
    students = models.JSONField(default=list, blank=True)  # Stores a list of strings
